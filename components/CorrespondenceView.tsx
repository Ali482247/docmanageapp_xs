...
import { getCorrespondenceById, getUsers, assignExecutor, assignInternalEmployee, submitForReview, approveReview } from '../services/api';
import { useAuth } from '../hooks/useAuth';
import { UserRole, CorrespondenceStage, getStageDisplayName } from '../constants';
import { ArrowLeftIcon, UserGroupIcon, PaperAirplaneIcon, CheckBadgeIcon, CheckIcon, ClockIcon, XMarkIcon } from './icons/IconComponents';
...
    }, [id]);
    ...
    const handleSubmitForReview = async () => {
        if (correspondence) {
            setLoading(true);
            try {
                const updatedDoc = await submitForReview(correspondence.id);
    ...
    const handleApproveReview = async () => {
        if (correspondence) {
            ...
            try {
                ...
                // Если в ответе пришел обновленный документ (значит, это был последний согласующий)
                if (result.id) {
                    setCorrespondence(result);
                } else {
                    fetchData(); // Иначе просто обновляем данные
                }
            } catch(err: any) { alert(`Xatolik: ${err.message}`); }
            finally { setLoading(false); }
        }
...
    const canSubmitForReview = (currentUser.id === correspondence.mainExecutor?.id || currentUser.id === correspondence.author.id) && correspondence.stage === CorrespondenceStage.DRAFTING;
    ...
    const canApprove = correspondence.stage === CorrespondenceStage.FINAL_REVIEW && isUserAReviewer;
    
    return (
        <>
            <button
