...
import { getCorrespondenceById, getUsers, assignExecutor, assignInternalEmployee, submitForReview, approveReview, rejectReview, signDocument } from '../services/api';
import { useAuth } from '../hooks/useAuth';
import { UserRole, CorrespondenceStage, getStageDisplayName } from '../constants';
import { ArrowLeftIcon, UserGroupIcon, PaperAirplaneIcon, CheckBadgeIcon, CheckIcon, ClockIcon, XMarkIcon, XCircleIcon, PencilIcon } from './icons/IconComponents';
...
const CorrespondenceView: React.FC = () => {
    ...
    const canAssign = currentUser.role === UserRole.Boshqaruv;
    ...
    const canSubmitForReview = (currentUser.id === correspondence.mainExecutor?.id || currentUser.id === correspondence.author.id) && correspondence.stage === CorrespondenceStage.DRAFTING;
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="p-4 border border-white/20 rounded-lg bg-black/20">
                            ...
                            <div className="mt-2 space-y-2">
                                {canAssign && (
                                    <button onClick={() => setShowAssignModal(true)} className="w-full flex items-center justify-center gap-2 px-4 py-2 text-white bg-blue-600 rounded-lg hover:bg-blue-700">
                                        <UserGroupIcon className="w-5 h-5" /> Ijrochini tayinlash
                                    </button>
                                )}
                                {canSubmitForReview && (
                                    <button onClick={handleSubmitForReview} disabled={loading} className="w-full flex items-center justify-center gap-2 px-4 py-2 text-white bg-blue-600 rounded-lg hover:bg-blue-700">
                                        <PaperAirplaneIcon className="w-5 h-5" /> Kelishuvga yuborish
                                    </button>
                                )}
                                {canApproveOrReject && (
                                    <div className="grid grid-cols-2 gap-2">
                                        <button onClick={handleRejectReview} disabled={loading} className="w-full flex items-center justify-center gap-2 px-4 py-2 text-white bg-red-600 rounded-lg hover:bg-red-700">
                                            <XCircleIcon className="w-5 h-5" /> Rad etish
                                        </button>
                                        <button onClick={handleApproveReview} disabled={loading} className="w-full flex items-center justify-center gap-2 px-4 py-2 text-white bg-emerald-600 rounded-lg hover:bg-emerald-700">
                                            <CheckBadgeIcon className="w-5 h-5" /> Tasdiqlash
                                        </button>
                                    </div>
                                )}
                                {canSign && (
                                    <button onClick={handleSignDocument} disabled={loading} className="w-full flex items-center justify-center gap-2 px-4 py-2 text-white bg-purple-600 rounded-lg hover:bg-purple-700">
                                        <PencilIcon className="w-5 h-5" /> Imzolash
                                    </button>
                                )}
                                {canDelegateInternal && (
                                    <>
